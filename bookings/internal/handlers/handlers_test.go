...
func TestRepository_PostReservation(t *testing.T) {
	...
	if rr.Code != http.StatusOK {
		t.Errorf("Reservation handler returned wrong response code for invalid form: got %d, wanted %d", rr.Code, http.StatusOK)
	}
	...
	// test for failure to insert room_restriction into database
	reservation = models.Reservation{
		RoomID: 1000,
	}
	...
	handler.ServeHTTP(rr, req)
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("PostReservation handler returned wrong response code for failure to insert room_restriction: got %d, wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}
}
